...
class JobExecution(Base):
    ...
    status = Column(
        Enum("queued", "running", "success", "failure", "timed_out", "dead_letter", name="execution_status_enum"),
        nullable=False,
    ...
    worker_id = Column(String(255), nullable=True)  # Celery worker that executed the job
    duration_ms = Column(Integer, nullable=True)  # Execution duration in milliseconds
    error = Column(Text, nullable=True)  # Error message if failed
    attempt = Column(Integer, nullable=False, server_default="1")  # Retry attempt number
