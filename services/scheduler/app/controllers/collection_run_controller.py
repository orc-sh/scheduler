...
# Run endpoints
@router.post(
    "/collections/{collection_id}/runs", response_model=CollectionRunResponse, status_code=status.HTTP_201_CREATED
)
...
@router.get("/collections/{collection_id}/runs", response_model=List[CollectionRunResponse])
...
async def run_collection(
    ...
    # Verify user has access
    collection_service = get_collection_service(db)
    collection = collection_service.get_collection(collection_run.collection_id)
    if not collection:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Configuration not found for this run",
    ...
    if collection_run.status == "running":
        raise HTTPException(
    ...
    if collection_run.status in ("completed", "failed"):
        collection_run.status = "pending"
        collection_run.started_at = None
...
async def get_collection_report(
    ...
    run = collection_service.get_collection_run(report.collection_run_id)
    if not run:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
